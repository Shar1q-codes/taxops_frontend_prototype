...
export function IncomeModule({ engagementId }: { engagementId: string }) {
  const { token } = useAuth();
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const data = await taxopsApi.fetchIncomeFindings(token, engagementId);
      setFindings(data);
    } catch (err) {
      const message = err instanceof Error ? err.message : "Failed to load income findings.";
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <Card>
      <CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h2 className="text-xl font-semibold text-slate-900">Income checks</h2>
          <p className="text-sm text-slate-600">Duplicate income transaction detection using TB/GL.</p>
        </div>
      ...
      <CardContent className="space-y-3">
        {loading ? (
          ...
          <div className="flex items-center gap-2 rounded-md border border-dashed border-slate-300 bg-slate-50 px-3 py-2 text-sm text-slate-700">
            <AlertCircle className="h-4 w-4" />
            No income findings yet. Upload TB + GL in Books, then refresh.
          </div>
        )}
      </CardContent>
    </Card>
